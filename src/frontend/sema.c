...
static ScmdTypeKind check_expr(ScmdExpr *e,const SymVec *locals,const SymVec *globals,const NameVec *functions,const char *path,int *errors){
    ...
    switch(e->kind){
        ...
        case EXPR_UNARY:{
            ScmdTypeKind a=check_expr(e->as.unary.value,locals,globals,functions,path,errors);
            ...
            break;}
        case EXPR_BINARY:{
            ScmdTypeKind a=check_expr(e->as.binary.lhs,locals,globals,functions,path,errors);
            ScmdTypeKind b=check_expr(e->as.binary.rhs,locals,globals,functions,path,errors);
...
static void collect_locals(ScmdStmt *s,SymVec *locals,const SymVec *globals,const NameVec *functions,const char *path,int *errors){
    for(;s;s=s->next){
        if(s->kind==STMT_VAR_DECL){
            ScmdTypeKind it=check_expr(s->as.var_decl.init,locals,globals,functions,path,errors);
            ...
            if(s->as.if_stmt.then_block)collect_locals(s->as.if_stmt.then_block->as.block_scope.first,locals,globals,functions,path,errors);
            if(s->as.if_stmt.else_block)collect_locals(s->as.if_stmt.else_block->as.block_scope.first,locals,globals,functions,path,errors);
        } else if(s->kind==STMT_WHILE){ if(s->as.while_stmt.body)collect_locals(s->as.while_stmt.body->as.block_scope.first,locals,globals,functions,path,errors); }
        else if(s->kind==STMT_FOR){
            if(s->as.for_stmt.init)collect_locals(s->as.for_stmt.init,locals,globals,functions,path,errors);
            if(s->as.for_stmt.body)collect_locals(s->as.for_stmt.body->as.block_scope.first,locals,globals,functions,path,errors);
        } else if(s->kind==STMT_BLOCK_SCOPE)collect_locals(s->as.block_scope.first,locals,globals,functions,path,errors);
    }
}

static void check_stmt(ScmdStmt *s,const SymVec *locals,const SymVec *globals,const NameVec *functions,const NameVec *blocks,
    ...
    for(;s;s=s->next){
        switch(s->kind){
            ...
            case STMT_ASSIGN:{
                ...
                ScmdTypeKind rhs=check_expr(s->as.assign.value,locals,globals,functions,path,errors);
                ...
                break;}
            case STMT_IF:{ScmdTypeKind c=check_expr(s->as.if_stmt.cond,locals,globals,functions,path,errors);if(c!=SCMD_TYPE_BOOL){scmd_error_at(path,s->line,s->col,"if condition must be bool, found %s",type_name(c));(*errors)++;}
                if(s->as.if_stmt.then_block)check_stmt(s->as.if_stmt.then_block->as.block_scope.first,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);
                if(s->as.if_stmt.else_block)check_stmt(s->as.if_stmt.else_block->as.block_scope.first,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);break;}
            case STMT_WHILE:{ScmdTypeKind c=check_expr(s->as.while_stmt.cond,locals,globals,functions,path,errors);if(c!=SCMD_TYPE_BOOL){scmd_error_at(path,s->line,s->col,"while condition must be bool, found %s",type_name(c));(*errors)++;}
                if(s->as.while_stmt.body)check_stmt(s->as.while_stmt.body->as.block_scope.first,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);break;}
            case STMT_FOR:{if(s->as.for_stmt.init)check_stmt(s->as.for_stmt.init,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);if(s->as.for_stmt.cond){ScmdTypeKind c=check_expr(s->as.for_stmt.cond,locals,globals,functions,path,errors);if(c!=SCMD_TYPE_BOOL){scmd_error_at(path,s->line,s->col,"for condition must be bool, found %s",type_name(c));(*errors)++;}}
                if(s->as.for_stmt.step)check_stmt(s->as.for_stmt.step,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);
                if(s->as.for_stmt.body)check_stmt(s->as.for_stmt.body->as.block_scope.first,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1);break;}
            ...
            case STMT_BLOCK_SCOPE: check_stmt(s->as.block_scope.first,locals,globals,functions,blocks,current_fn,path,errors,edges,fn_count,in_named_block,records,nested_depth+1); break;
            case STMT_RECORD:
...
bool scmd_sema_check(const char *path,ScmdProgram *program){
    int errors=0; SymVec globals={0}; NameVec functions={0},blocks={0};
    for(ScmdGlobal *g=program->globals;g;g=g->next){
        ScmdTypeKind it=check_expr(g->init,&(SymVec){0},&globals,&functions,path,&errors); ScmdTypeKind rt=g->declared_type==SCMD_TYPE_UNKNOWN?it:g->declared_type;
        ...
        g->resolved_type=rt; if(sv_find(&globals,g->name)>=0){scmd_error_at(path,g->line,g->col,"duplicate global '%s'",g->name);errors++;}else sv_push(&globals,g->name,rt);
    }
    for(ScmdFunction *f=program->functions;f;f=f->next){if(nv_find(&functions,f->name)>=0){scmd_error_at(path,f->line,f->col,"duplicate function '%s'",f->name);errors++;}else nv_push(&functions,f->name);}
    ...
    for(ScmdFunction *f=program->functions;f;f=f->next){SymVec locals={0};collect_locals(f->body,&locals,&globals,&functions,path,&errors);check_stmt(f->body,&locals,&globals,&functions,&blocks,f->name,path,&errors,edges,fn_count,false,NULL,0);free(locals.items);}
    for(ScmdBlock *b=program->blocks;b;b=b->next){
        ...
        SymVec no_locals={0}; check_stmt(b->body,&no_locals,&globals,&functions,&blocks,NULL,path,&errors,edges,fn_count,true,&records,0); free(records.items);
    }
    ...
    free(edges);free(state);free(globals.items);free(functions.items);free(blocks.items);return errors==0;
}
